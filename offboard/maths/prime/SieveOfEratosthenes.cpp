...
vector<unsigned int>* EratosSieveToSqrt( const unsigned int maxValue, bool* const sieveMap )
{
	// We'll use a dirty ol' C-style array for this - vectors buy
	// us nothing here and may result in a (very slight) performance drop.

	// Note indexes 0 and 1 are uninitialized,
	// we could initialize them to true for
	// clarity, but this comment suffices.

	// Initialize the rest to be potentially prime
	// Note we are ignoring multiples of 2 - in fact we don't
	// even initialize them - urgh! - all in the name of performance
	...
	// NOTE: In terms of memory usage it might be better to
	...
	// the push back is expensive and could initialize
	...
	// Must protect against the case where we ask for
	// all the primes up to 1
	if ( maxValue >= 2 )
	{
		// Must add 2 as a prime as we factor out multiples of 2 below
		primes->push_back(2);

		// Loop over the *initialized* odd values, ignoring
...
vector<unsigned int>* FirstNEratosPrimes( const unsigned int numberRequired )
{
	...
	vector<unsigned int>* primes = EratosSieveToSqrt( upperBound, sieve );
...
vector<unsigned int>* EratosPrimesUpTo( const unsigned int maxValue )
{
	// See FirstNEratosPrimes() for relevant comments
	bool* sieve = new bool[ maxValue + 1 ];
	vector<unsigned int>* primes = EratosSieveToSqrt( maxValue, sieve );
...
unsigned int NextPrimeGreaterOrEqualTo( const unsigned int number )
{
	...
	const auto_ptr< const vector<unsigned int> > allPrimesUpToNumber( EratosPrimesUpTo( number ) );
	if ( !allPrimesUpToNumber->empty() && ( allPrimesUpToNumber->back() == number ) )
		// If number itself prime, it will be the last entry,
		// just return it
		return number;

	cout << "\nSize:" << allPrimesUpToNumber->size();
