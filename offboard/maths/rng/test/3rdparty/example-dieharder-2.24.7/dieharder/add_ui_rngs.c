...
void add_ui_rngs()
{
 ...
 i++;
 // ***************************** Phil's ranq1 attempt *********************************
 types[i] = ( get_gsl_rng_ranq1() );
 if(verbose) printf("# add_my_types():  Added type %d = %s\n",i,types[i]->name);
 i++;
 // ***************************** Phil's Mersenne attempt *********************************
 types[i] = ( get_gsl_rng_mersenne() );
 if(verbose) printf("# add_my_types():  Added type %d = %s\n",i,types[i]->name);
 i++;
 // ***************************** Phil's ran attempt *********************************
 types[i] = ( get_gsl_rng_ran() );
 if(verbose) printf("# add_my_types():  Added type %d = %s\n",i,types[i]->name);
 i++;
 // ***************************** Phil's dodgy Halton attempt *********************************
 types[i] = ( get_gsl_rng_halton() );
 if(verbose) printf("# add_my_types():  Added type %d = %s\n",i,types[i]->name);
 i++;
